...
from database.custom_embedchain.loaders.inner_tube import InnerTube
...
class YouTube:
    ...
    async def async_vid_info(self):
        ...
        if self._vid_info:
            return self._vid_info
        innertube = InnerTube(use_oauth=self.use_oauth, allow_cache=self.allow_oauth_cache)
        innertube_response = await innertube.async_player(self.video_id)
        self._vid_info = innertube_response
        return self._vid_info
    ...
    async def get_title(self) -> str:
        """Get the video title asynchronously.
        ...
        if self._title:
            return self._title
        try:
            # Assuming there's an async version of this
            # self.vid_info = 
            result = await self.async_vid_info()
            self._title = result['videoDetails']['title']
            # self._title = self.vid_info['videoDetails']['title']
        except KeyError:
            # Check_availability will raise the correct exception in most cases
            #  if it doesn't, ask for a report.
            await self.check_availability()  # This method needs to be async too
            raise exceptions.PytubeError(
                (
                    f'Exception while accessing title of {self.watch_url}. '
                    'Please file a bug report at https://github.com/pytube/pytube'
