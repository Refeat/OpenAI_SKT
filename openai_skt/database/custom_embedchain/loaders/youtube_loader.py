...
from database.custom_embedchain.loaders.youtube import YouTube
...
class YoutubeLoader(BaseLoader):
    ...
    async def async_load(self) -> List[Document]:
        ...
        if self.add_video_info:
            # Get more video meta info
            # Such as title, description, thumbnail url, publish_date
            video_info = await self._async_get_video_info()
            metadata.update(video_info)
        try:
            transcript_list = YouTubeTranscriptApi.list_transcripts(self.video_id)
        except TranscriptsDisabled:
            return []
        # detected_language = "ko"  # Default to English if language detection fails

        try:
            # Detect the language of the transcript
            for transcript in transcript_list:
                # detected_language = transcript.language_code
                # transcript_pieces = transcript.translate('ko').fetch()
                transcript_pieces = transcript.fetch()
                break
        except Exception as e:
            print(f"Language detection failed: {str(e)}")
    ...
    async def _async_get_video_info(self) -> dict:
        """Get important video information asynchronously.
        ...
        yt = YouTube(f"https://www.youtube.com/watch?v={self.video_id}")
        # title = await yt.get_title()  # Assuming yt.title is an async property
        video_info = {
            "title": await yt.get_title() or "Unknown",
            # "description": yt.description or "Unknown",
            # "view_count": yt.views or 0,
            # "thumbnail_url": yt.thumbnail_url or "Unknown",
            # "publish_date": yt.publish_date.strftime("%Y-%m-%d %H:%M:%S")
